...
<details>
<summary style="cursor : pointer;">성장경험</summary>

무한 스크롤,경고창 기능등을 커스텀 훅으로 만들어 개발 생산성을 높였으며 공공 API(영양소)를 통한 서버 통신을 경험하였고
특히 SNS특성상 이미지 로딩이 길어지는 문제로 Next/image를 활용한 크기, 용량 조절로 렌더  1.8에서 0.16초로 이미지 렌더링 속도를 단축했습니다.<br/>
또 URL접근이나 스토리지 변경을 통한 보안 문제를 공식문서를 통해 Next.js13부터 더욱 편리해진 middleware를 통해 토큰 검사를 통한 보안을 높일 수 있었습니다.<br/>

새로운 버전의 기술들을 도입하는 건 처음이었고 실제로 정보가 매우 적다보니 공식 기술 블로그와 문서 등을 활용하여 코드를 작성해 보는 경험을 느낄 수 있었습니다.
 </details>
<details>
<summary style="cursor : pointer;">회고 펼치보기</summary>
  원래 일찍 시작했었던 프로젝트를 다시 새로나온 NEXT.JS 13 버전과 더불어 공부한 퍼블리싱 지식으로 아예 새롭게 시작했다.<br>
Next.js와 react, mongoDB를 사용해서 프로젝트 만든 후 vercel을 통해 배포를 했다<br>
